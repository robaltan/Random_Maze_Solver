...
class Environment:
    ...
    def step(self, state, action):
       ...
       # Function to calculate weight given a transition 
    def calculate_transition_weight(self, transition, weight_epsilon):
        # Extract (S, A, R, S')
        ...
        q_value = self.dqn.q_network.forward(state_tensor)[action]
        # Get the Q-function values with the Q-Network and the next state
        q_values_prime = self.dqn.q_network(prime_state_tensor)
